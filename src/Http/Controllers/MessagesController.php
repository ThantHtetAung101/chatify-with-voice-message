...
class MessagesController extends Controller
{
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = Message::join('users',  function ($join) {
            ...
            ->where(function ($q) {
                $q->where('ch_messages.from_id', 1)
                    ->orWhere('ch_messages.to_id', 1);
            })
            ->where('users.id', '!=', 1)
    ...
    public function sendPushNotification($title, $message, $userId = null, $imgUrl = null)
    {
        ...
        $fcm = Fcmtokeykey::select('token')->where('user_id', $userId)->whereNotNull('token')->where('token', '!=', 'BLACKLISTED')->groupBy('token')->latest()->get();
